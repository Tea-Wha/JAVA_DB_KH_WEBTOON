    ...
    public void insertDataFromCSV(String filePath) {
        ...
        try (Connection conn = Common.getConnection();
            ...
            while (scanner.hasNextLine()) {
                ...
                // 구독자수 (NaN 값 처리)
                if (data[5].trim().isEmpty() || data[5].trim().equalsIgnoreCase("NaN")) {
                    psmt.setNull(6, Types.INTEGER);
                } else {
                    psmt.setInt(6, Integer.parseInt(data[5].trim()));
                }

                // 조회수 (NaN 값 처리)
                if (data[6].trim().isEmpty() || data[6].trim().equalsIgnoreCase("NaN")) {
                    psmt.setNull(7, Types.INTEGER);
                } else {
                    psmt.setInt(7, Integer.parseInt(data[6].trim()));
                }
